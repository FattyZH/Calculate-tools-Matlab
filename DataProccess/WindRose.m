...
elseif LegendType==1                                                       % If legend type is colorbar
    ...
    hc = colorbar('YTick',vwinds,'location','southoutside');                                              % The values shown in the colorbar are the intenisites.
    set(get(hc,'ylabel'),'string',lablegend);
end
...
function [color,axs] = DrawPatches(N,n,vwinds,count,colorfun,figcolor,min_radius,colors,inverse,axs)
    ...
    for i=1:length(circles)
        text(radius(i)*c,radius(i)*s,[num2str(circles(i)) '%'],'HorizontalAlignment',ha,'verticalalignment',va,'color',TextColor,'fontsize',20,'fontname','times'); % display the labels for each circle
    end
...
%         text(rmin*c,rmin*s,'0%','HorizontalAlignment',ha,'verticalalignment',va,'color',TextColor,'fontsize',20,'fontname','times'); % display the labels for each circle
    end
    
function CardinalLabels(circlemax,TextColor,labels)
    text( circlemax,0,[' ' labels.E],'HorizontalAlignment','left'  ,'verticalalignment','middle','color',TextColor,'fontsize',20,'fontname','times'); % East  label
    text(-circlemax,0,[labels.W ' '],'HorizontalAlignment','right' ,'verticalalignment','middle','color',TextColor,'fontsize',20,'fontname','times'); % West  label
    text(0, circlemax,labels.N      ,'HorizontalAlignment','center','verticalalignment','bottom','color',TextColor,'fontsize',20,'fontname','times'); % North label
    text(0,-circlemax,labels.S      ,'HorizontalAlignment','center','verticalalignment','top'   ,'color',TextColor,'fontsize',20,'fontname','times'); % South label
